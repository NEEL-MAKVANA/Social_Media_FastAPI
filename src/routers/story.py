...
def get_story_within_time(user_id:str):
    ...
    find_user_story_first=db.query(Story).filter((Story.user_id == user_id) & (curr_time < (Story.created_at + timedelta(minutes=1)))).first()

    find_user_story=db.query(Story).filter((Story.user_id == user_id) & (curr_time < (Story.created_at + timedelta(days=1))))
    ...
    if not find_user_story_first:
        ...
        db.commit()
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail="Story has been expired")
